...
class FSS_Email_Handler {
    ...
    public function send_sequential_emails($entry_id, $form_id) {
        ...
        // Utiliser un transient pour détecter les appels en double (expire après 1 heure)
        $transient_key = "fss_email_sent_{$entry_id}";
        $already_sent = get_transient($transient_key);

        if ($already_sent !== false) {
            error_log("[FSS] Entry {$entry_id} | SKIPPED | Reason: Email already sent (duplicate hook call prevented)");
            error_log("[FSS] === END Processing Entry {$entry_id} ===");
            return; // Ne pas renvoyer d'email
        }
        ...
        if ($result) {
            ...
            // Marquer l'entry comme traitée pour éviter les envois multiples
            $sent_info = array(
            ...
            set_transient($transient_key, $sent_info, HOUR_IN_SECONDS);
            error_log("[FSS] Entry {$entry_id} marked as processed (protection against duplicate sends for 1 hour)");
